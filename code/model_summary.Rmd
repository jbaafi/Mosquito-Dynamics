...
### Model parameters 
This is where I show the relationship between temperature and rainfall on entomological parameters used in the model.

1. Egg laying rate function

```{r, echo=FALSE, message=FALSE}
temp <- seq(5, 40)
beck <-  0.443111*exp(-((temp - 34.388928)^2)/185.611107)
...
oviposition #+ lims(y = c(0, 0.5)) 
ggsave(path = "/Users/jbaafi/Desktop", filename = "ovi.png")
```

2. Mortality rate functions
...
mort <- ggplot(data = df2, aes(x=temp, y=rate, col = Stage))+
  ...
  ylab("Mortality rate (days)") +
  ggtitle("Stage specific mortality rates") +
  theme_classic() +
...
mort
ggsave("mortality.png", mort)

param <- oviposition / mort

param
ggsave("param.png", param, path = "/Users/jbaafi/Desktop")
```




3. Development rate functions (depends on both temperature and rainfall). We show this relationship by plotting a contour plot.

a. Egg development rate

Comment: From literature, mortality rate can be treated as reciprocal of development time in a given stage (this is plotted in a different file called dev_rate_func.Rmd). Another data from Beck-Johnson also shows mortality as a function of temperature. Each data I use give me a different rate for development. Discuss with Amy which data will be much appropriate to use for this model.
...
#R <- seq(4, 320, length.out = 100)
#R <- seq(0, 60, length.out = 100)
...
dev <- ggplot(df, aes(x = temp, y = R, fill = rho_E)) +
  geom_tile() +
  labs(x = "Temperature", y = "Rainfall", title = "Egg development time", fill = "Days") +
  theme_bw() +
 # coord_cartesian(ylim = c(ylim_min, ylim_max)) +
...
dev
ggsave("dev.png", dev, path = "/Users/jbaafi/Desktop")

```



b. Larvae development rate 
...
dev.larv <- ggplot(df, aes(x = temp, y = R, fill = rho_L)) +
  geom_tile() +
  labs(x = "Temperature", y = "Rainfall", title = "Larvae development time", fill = "Days") +
  theme_bw() +
 # coord_cartesian(ylim = c(ylim_min, ylim_max)) +
  scale_fill_gradient(low = "lightyellow", high = "red")  +
  theme(axis.text.x = element_text(colour = "grey20", size = 13, angle = 0,
...
dev.larv
ggsave("dev.larv.png", dev.larv, path = "/Users/jbaafi/Desktop")

```


c. Pupae development rate
...
dev.pupa <- ggplot(df, aes(x = temp, y = R, fill = rho_P)) +
  geom_tile() +
  labs(x = "Temperature", y = "Rainfall", title = "Pupae development time", fill = "Days") +
  theme_bw() +
 # coord_cartesian(ylim = c(ylim_min, ylim_max)) +
  scale_fill_gradient(low = "lightyellow", high = "red") +
  theme(axis.text.x = element_text(colour = "grey20", size = 13, angle = 0,
...
dev.pupa
ggsave("dev.pupa.png", dev.pupa, path = "/Users/jbaafi/Desktop")
```


### Model results

1. Model with constant environmental variables
...
The above Fig. shows larval and adult density over time. I chose to plot only these stages just because they are the stages that control strategies are normally targeted. Communities mostly spray larvicides or adulticides to kill larvae or adults depending on the period of the year. For constant environmental parameters, the model fail to explain the seasonal pattern mostly seen in mosquito population. A better approach will be to introduce seasonal weather into the model and that is what we show in the next section. 

2. Model with seasonal environmental variables (temperature and rainfall)
...
upper_limit <- max(df$y) * 1.1

adult <- ggplot(df, aes(x, y, col = Region)) +
  geom_line(lwd = 1) +
  labs(title = "Number of female adult mosquitoes in different regions",
       x = "",
       y = "Number of adults") +
  theme_classic() +
